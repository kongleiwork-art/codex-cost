...
成本模型来自受控实验（641 次测量、32 组，research/refit.py 联合拟合）：
...
# sol 的倍数差了一个数量级（fresh ~2.9×、输出 ~5.7×、每请求 ~8×），单一乘数下
# 等效倍数完全取决于调用的构成，怎么都对不上。

# (每 1% 的 fresh token 数, 每 1% 的缓存输入 token 数,
#  每 1% 的输出侧 token 数, 每次请求成本%)
#
# 与 Sources/Budget.swift 保持一致，来龙去脉见那边的注释。简言之：对全部 641 次
...
COEF = {
    ...
    "gpt-6-astra":   (14_545, 168_559,  2_364, 0.2615),   # 缓存列按 sol 比例设定
}
