...
- **Sol 测得最扎实** —— 164 个回归点联合拟合，RMS 0.39（舍入噪声下限 0.29）。每一组大上下文实验的预测都落在实测 1 个百分点以内（见下表）。但真实使用仍比模型贵约 15%（近两月 7%~9%），这个缺口还没解决。
...
第 5 版在每一组上都落在实测 1 个百分点以内。它仍解释不了的是：193 段真实使用比预测贵约 15%（近两月 7%~9%）。改用真实数据直接拟合会把缓存定到约 30 万 tok/1%，但拟合误差是 6.8，而实验拟合是 0.39 —— 所以发布的是实验值，这个缺口留作待查。
