...
**让缓存失效的是空闲时间。** 在这个账号自己的会话里，距上一次请求不到 1 分钟时，缓存失效的概率约 1%；空闲 10–30 分钟约四分之一；超过 1 小时接近九成。一次失效会把整段上下文按新增输入重读 —— 15 万 token 的 Sol 会话，要花 5 小时窗口的约 3.5%，而不是 0.3%。近 30 天里缓存失效吃掉了这个账号约 11% 的额度，所以面板会在你回到大会话时提醒。
