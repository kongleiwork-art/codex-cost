...
第二个需要成本模型。为了拿到它，做了 **32 个实验组、641 次受控调用**，每次只改一个变量。原始数据和实验框架都在 [`research/`](research/)。
...
**Astra 不能用一个倍数描述。** 相对 Sol，它的 fresh 输入贵约 2.9 倍，输出贵约 5.7 倍（每请求底价看起来贵约 8 倍，但这个数依赖一个假定的缓存费率）—— 所以任何"Astra 贵 N 倍"的说法都会随模型说话多少而变。让它干短平快的活还行，让它写长推理不行。
...
Sol 的四个系数用全部 641 次测量做联合非负最小二乘拟合（[`research/refit.py`](research/refit.py)）。5.5 和 Terra 在现有分辨率下与 Sol 分不出来，由 Sol 缩放而来。\*Astra 的缓存费率在现有数据里测不出来，按 Sol 的缓存/fresh 比例设定，它的每请求数值会随这个假定变动；专门的 `astra/bigctx` 实验组已定义，尚未运行。
