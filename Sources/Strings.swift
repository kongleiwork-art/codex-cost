...
enum L {
    ...
    static func resumeCost(_ pct: Double) -> String {
        let p = String(format: "%.1f", pct)
        return s("接着这段会话每轮约 \(p)% 额度 —— 花的是上下文大小，与缓存过期无关",
                 "About \(p)% per turn if you continue — you pay for context size, not cache age")
    }
